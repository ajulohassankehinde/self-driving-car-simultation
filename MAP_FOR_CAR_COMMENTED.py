# -*- coding: utf-8 -*-
"""
Created on Sun Sep 30 03:04:01 2018
...
from AI_FOR_CAR_COMMENTED import brainOfCar
...
last_x = 0 #coordinates of x axis(keep updating)
last_y = 0#coordinates of y axis (keep updating )
n_points = 0 # the total number of points in the last drawing
length = 0 # the length of the last drawing

# Getting our AI, which we call "brain", and that contains our neural network that represents our Q-function
brain = brainOfCar(5,3,0.9) # 5 sensors, 3 actions, gama = 0.9 (3 actions = straight,left and right )
action2rotation = [0,20,-20] # action = 0 => no rotation, action = 1 => rotate 20 degres(clockwise - right), action = 2 => rotate -20 degres (anticlockwise - left)
last_reward = 0 # initializing the last reward (keeps updating)
...
def init():
   ...
   # first_update = False #  to initialize the map only once
...
class Car(Widget):
    ...
    def move(self, rotation):
        ...
        self.angle = self.angle + self.rotation # updating the angle with rotation
        self.sensor1 = Vector(30, 0).rotate(self.angle) + self.pos # updating the position of sensor 1 (30 units because our sensor has an area of 20 units suqare...so we check area upfront by 30 units n keep taking steps)
        self.sensor2 = Vector(30, 0).rotate((self.angle+30)%360) + self.pos # updating the position of sensor 2 (positive angle - towards right )
        self.sensor3 = Vector(30, 0).rotate((self.angle-30)%360) + self.pos # updating the position of sensor 3 (negative angle - towards left)
        self.signal1 = int(np.sum(sand[int(self.sensor1_x)-10:int(self.sensor1_x)+10, int(self.sensor1_y)-10:int(self.sensor1_y)+10]))/400. # getting the signal received by sensor 1 (density of sand around sensor 1) (updating the signals : x+10 and x-10 * y+10 - y-10)
        ...
        #checking all the constraints for which we can reward a negative penalty
        if self.sensor1_x > longueur-10 or self.sensor1_x<10 or self.sensor1_y>largeur-10 or self.sensor1_y<10: # if sensor 1 is out of the map (the car is facing one edge of the map) (checking if : car is beyond right edge ...ie (longiteuer -10), beyond -x x axis...ie x-10 , above largeruur y-10 annd finally below y axis)
...
class Game(Widget):
    ...
    def update(self, dt): # the big update function that updates everything that needs to be updated at each discrete time t when reaching a new state (getting new signals from the sensors)
        ...
        orientation = Vector(*self.car.velocity).angle((xx,yy))/180. # direction of the car with respect to the goal (if the car is heading perfectly towards the goal, then orientation = 0) (orientation : veltocity * angle)
        last_signal = [self.car.signal1, self.car.signal2, self.car.signal3, orientation, -orientation] # our input state vector, composed of the three signals received by the three sensors, plus the orientation and -orientation ( orientation just rotates the car towards right. -Orientation is included to bring balanace to the car...suppose)
        #if our car only loops towards right direction, it may never tend to reach the goal...sso we need to add the -orientation so that it can always look to loop towards left if right is not giving the goal state
...
class MyPaintWidget(Widget):
    ...
    def on_touch_down(self, touch): # putting some sand when we do a click
    ...
    def on_touch_move(self, touch): # putting some sand when we move the mouse while pressing left button of the mouse
        global length,n_points,last_x,last_y
...
class CarApp(App):
    ...
    def build(self): # building the app
        ...
        clearbtn.background_color = (0,0,0,0)
        savebtn.background_color = (0,0,0,0)
        loadbtn.background_color = (0,0,0,0)
